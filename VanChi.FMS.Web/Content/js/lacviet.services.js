var lacviet_services = {
    ...
    checkAll: function (idCheckbox, classCheckbox, strButtonId) {
        ...
        lacviet_services.refreshButton(classCheckbox, strButtonId);
    },
    ...
    refreshRegisterTotal: function (url) {
        ...
        lacviet_services.postAsync(url, JSON.stringify(),
            function (result) {
    ...
    postAutocomplete: function (idInput, urlMethod, dataString, onPostSuccess, onPostError, onPostSelect) {
        $(idInput).autocomplete({
            source: function (request, response) {
                lacviet_services.postAsync(urlMethod, dataString, onPostSuccess, onPostError);
            },
        ...
        $.getJSON(urlUserInfoGetByUserName, { userNames: textUsers.val() }, function (data) {
            ...
            $.each(data, function (i, user) {
                if ($("#" + id).val().indexOf(user.Value) <= -1 && $("#" + id).val() != user.Value && (type != "single" || (type == "single" && $("#" + id).val() == ''))) {
                    items += "<li class='search-choice'><span>" + user.Text + "</span><a class='search-choice-close' onclick='lacviet_services.deletePickUser(this)' elementId='" + id + "' userName='" + user.Value + "'></a></li>";
                    if ($("#" + id).val() == "")
                        $("#" + id).val(user.Value);
                    else
